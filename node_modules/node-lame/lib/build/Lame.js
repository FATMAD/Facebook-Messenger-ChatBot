...
class Lame {
    ...
    execProgress(inputFilePath, args, type) {
        ...
        const progressStdout = (data) => {
            ...
            if (data.length > 6) {
                if (type == "encode" && data.search("Writing LAME Tag...done") > -1) {
                    ...
                    if (!isNaN(progress) && Number(progress) > this.status.progress) {
                        this.status.progress = Number(progress);
                    }
                    this.emitter.emit("progress", [this.status.progress, this.status.eta]);
                }
                else if (data.search(/^lame: /) > -1 || data.search(/^Warning: /) > -1 || data.search(/Error /) > -1) {
        ...
        const instance = child_process_1.exec("lame", args);
